...
public class VueElement extends JPanel {
	...
	/**
	 * La coordonnee en ordonnée de la vue du personnage par rapport a la grille
	 * (en nombre de cases).
	 */
	protected int y;
	...
	public VueElement(int tailleCase, int xGrille, int yGrille, int x, int y) {
		...
		final int xImage = this.xGrille + x * this.tailleCase
				+ DIFFERENCE_PLACEMENT_CASE_IMAGE;
		final int yImage = this.yGrille + y * this.tailleCase
				+ DIFFERENCE_PLACEMENT_CASE_IMAGE;
	...
	public void setDimensions(int tailleCase, int xGrille, int yGrille) {
		...
		final int xImage = this.xGrille + this.x * this.tailleCase
				+ DIFFERENCE_PLACEMENT_CASE_IMAGE;
		final int yImage = this.yGrille + this.y * this.tailleCase
				+ DIFFERENCE_PLACEMENT_CASE_IMAGE;
		this.setBounds(xImage, yImage, this.tailleImage + 1,
				this.tailleImage + 1);
	}
